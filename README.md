...
See [SECURITY.md](./SECURITY.md) for the threat model and honest isolation limits.

**Local subprocess execution is not a security boundary.** Prefer Docker when available.
...
GitHub Actions (`.github/workflows/ci.yml`) runs typecheck, mocked tests (no API keys), and builds server + client on push/PR to `main`.

LLM providers: **Gemini → Groq → Mock** (see `server/.env.example`).
